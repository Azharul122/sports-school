            ...
            {user &&
              users.map((sclass, index) => (
                <tr>
                  ...
                  <td>
                    <button
                      disabled={sclass.role == "instructor"}
                      onClick={() => makeInstructor(sclass._id, sclass.name)}
                      className={`bg-stone-500 text-white p-1 ${
                        sclass.role == "instructor" ? "opacity-50" : ""
                      }`}
                    >
                      Instructor
                    </button>{" "}
                    <button
                      disabled={sclass.role == "admin"}
                      onClick={() => makeAdmin(sclass._id, sclass.name)}
                      className={`text-white bg-slate-500 p-1 ${
                        sclass.role == "admin" ? "opacity-50" : ""
                      }`}
                    >
                      Admin
                    </button>
